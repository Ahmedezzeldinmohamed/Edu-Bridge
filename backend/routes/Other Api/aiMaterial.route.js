...
// Route لرفع الملفات
router.post("/upload-ai-material", upload.single("file"), async (req, res) => {
  try {
    if (!req.file) {
    ...
    const prompt = `قم بإنشاء 60 سؤالًا مع إجاباتها بناءً على النص التالي، بحيث يكون كل سؤال متبوعًا بإجابته مباشرة:\n\n${fileContent}`;
